...
#  virtual_machine_fqdn = join(".", [var.vm_name, var.ad_domain_name])
  ...
  allow_winrm_firewall = "New-NetFirewallRule -DisplayName 'Allow WinRM HTTP' -Name 'Allow_WinRM_HTTP' -Protocol TCP -LocalPort 5985 -Action Allow -Direction Inbound"
  reboot_command       = "shutdown -r -t 30"
  exit_code            = "exit 0"
  powershell_command   = "${local.adduser_command}; ${local.enable_command}; ${local.configure_command}; ${local.cleanup_command}; ${local.move_cdrom}; ${local.format_diskd}; ${local.extend_c}; ${local.allow_winrm_firewall}; ${local.reboot_command}; ${local.exit_code}"
}