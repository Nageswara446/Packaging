...
# Check if extend count is less than 3
if ($extendCount -lt $maxExtensionCount) {
    # Get the current date and time
    $currentDateTime = Get-Date
    $currentDateTimeFormatted = $currentDateTime.ToString('yyyy-MM-dd')
    Write-Host "CurrentDateTime : $currentDateTimeFormatted"

    # Calculate the new expiration date (current date + 48 hours)
    # $newExpirationDate = $currentDateTime.AddDays(2)
    ...
    $newExpirationDate = Get-NextBusinessDay -date $expirationDate -daysToAdd 2
    $newExpirationDateFormatted = $newExpirationDate.ToString('yyyy-MM-dd')
    ...
    $emailBody = "The extension count for VM $vmName has exceeded the limit of 3. Cannot extend the VM expiration date further."
    Send-ScriptNotificationEmail -TMUusername $TMUusername -TMUpassword $TMUpassword -Subject $emailSubject -Body $emailBody -Recipient $emailRecipient
